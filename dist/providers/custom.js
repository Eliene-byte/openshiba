    ...
    async chat(messages, options) {
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                throw new Error(`Custom provider API error: ${err.message ?? String(err)}`);
            }
        }
        throw new Error(`Custom provider request failed after ${MAX_RETRIES} retries: ${lastError.message ?? String(lastError)}`);
    }
    // ── Streaming chat ────────────────────────────────
    async *stream(messages, options) {
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                throw new Error(`Custom provider stream error: ${err.message ?? String(err)}`);
            }
        }
        throw new Error(`Custom provider stream failed after ${MAX_RETRIES} retries: ${lastError.message ?? String(lastError)}`);
    }
    // ── Models ────────────────────────────────────────
    async listModels() {
        try {
