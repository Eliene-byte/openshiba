...
export class OpenAIProvider {
    ...
    async chat(messages, options) {
        ...
        throw new Error(`OpenAI request failed after ${MAX_RETRIES} retries: ${lastError.message ?? String(lastError)}`);
    }
    // ── Streaming chat ────────────────────────────────
    async *stream(messages, options) {
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                throw new Error(`OpenAI stream error: ${err.message ?? String(err)}`);
            }
        }
        throw new Error(`OpenAI stream failed after ${MAX_RETRIES} retries: ${lastError.message ?? String(lastError)}`);
    }
    // ── Models ────────────────────────────────────────
    async listModels() {
        try {
