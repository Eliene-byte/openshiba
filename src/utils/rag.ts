...
export async function readFileContent(filePath: string): Promise<string> {
  ...
  if (!isSupportedExtension(resolvedPath)) {
    throw new Error(
      `Unsupported file type "${path.extname(resolvedPath)}". ` +
 ...
 * ```
 */
export function formatChunksForContext(chunks: string[], fileName: string): string {
 ...
 */
export async function attachFile(filePath: string): Promise<string> {
