 ...
 */
function wordDiff(a: string, b: string): Array<{ type: 'same' | 'add' | 'remove'; word: string }> {
  ...
  const result: Array<{ type: 'same' | 'add' | 'remove'; word: string }> = [];
  let i = m;
  let j = n;

  const backtrack: Array<{ type: 'same' | 'add' | 'remove'; word: string }> = [];
  while (i > 0 || j > 0) {
