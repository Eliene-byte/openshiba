...
export class GroqProvider implements ProviderInstance {
  ...
  async chat(messages: Message[], options: ChatOptions): Promise<ProviderResponse> {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        const isTimeout = (err as Error).message?.includes('timeout') ?? false;
        ...
        throw new Error(
          `Groq API error: ${(err as Error).message ?? String(err)}`,
        );
      }
    }

    throw new Error(
      `Groq request failed after ${MAX_RETRIES} retries: ${(lastError as Error).message ?? String(lastError)}`,
    );
  }
  ...
  async *stream(messages: Message[], options: ChatOptions): AsyncIterable<StreamChunk> {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        throw new Error(
          `Groq stream error: ${(err as Error).message ?? String(err)}`,
        );
      }
    }

    throw new Error(
      `Groq stream failed after ${MAX_RETRIES} retries: ${(lastError as Error).message ?? String(lastError)}`,
    );
  }
